...
export class Fracpane {
    ...
    constructor(options) {
        ...
        Fracpane.instances++;
        if (!options.title && Fracpane.instances > 1)
            this.title += `_${Fracpane.instances}`;
        this.id = `fracpane_${Fracpane.instances}_${this.title}`;
        this.position ??= { x: 10, y: 10 };
        if (this.persistent) {
            this.positionStore = persistentAtom(`${this.id}_position`, this.position, {
    ...
    async init() {
        if (typeof window === 'undefined')
            return console.warn('skipping fracpane server-side');
        ...
        if (this.persistent)
            this.dragOptions.position = this.position;
        ...
        const onDestroy = (mutationList, observer) => {
...
export const fracpane = async (options) => {
    return (await new Fracpane(options ?? {}).init());
};
